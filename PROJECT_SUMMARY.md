...
| **Tactical Board** | Pitch editor: formations, board types, draggable players (kit colours one tap each), arrows, shapes, pen, text, silhouettes, **multi-frame animation**, save/load, link to a match or training. **Material** (v276–v277), in five groups — Pilota · Marcadors (cons, platets, piques, anelles) · Agilitat (tanques baixes i altes, escala) · Porteries (mini, F7, F11) · Altres (maniquins, rebotador) — in a fixed colour palette, turnable to any angle, in 2D and 3D. Every item on a linked board is **counted** in the training's material list, by type and colour |
| **Notifications** | Feed of player actions with unread badge |
| **Sancions** | The Competition Committee's rulings for the squad's own FCF group: who is out for the next official jornada on both sides (matches still to serve), then the group archive (Vigents / Totes / Només nosaltres). Squad from the category bar; rulings against clubs are not shown (v273–v274) |
| **Golejadors** | Top scorers anywhere in the federation via four checkbox filters (season single-choice); nothing is read while a filter is open; sortable table; the club's public contact card per row (v273) |
